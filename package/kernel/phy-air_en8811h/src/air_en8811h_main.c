 ...
 *                  F U N C T I O N S
 ***********************************************************/
static void inline air_mdio_read_buf(struct phy_device *phydev, unsigned long address,
...
{
	int ret, reg_value, retry;

	ret = air_buckpbus_reg_write(phydev, 0x0f0018, 0x0);
	if (ret < 0)
		return ret;

	ret = air_buckpbus_reg_write(phydev, 0x0f0018, 0x1);
	if (ret < 0)
		return ret;
