...
static ssize_t common_proc(struct file *filp, const char *user_buf, size_t count, loff_t *offp)
{
	struct inode *ino = file_inode(filp);
	struct xpcs_priv *priv = PDE_DATA(ino);
	...
	if (strncmp(str[0], "rw", 2) == 0) {
		u32 devad, reg, val;
		ret = kstrtou32(str[1], 0, &devad);
		ret = kstrtou32(str[2], 0, &reg);
		if (str[3][0] != '\0'){
			ret = kstrtou32(str[3], 0, &val);
			xpcs_write(priv, devad, reg, val);
			printk("write pcs:%d reg:0x%x val:0x%x\n", devad, reg, val);
		} else {
			val = xpcs_read(priv, devad, reg);
			printk("read pcs:%d devad:0x%x reg:0x%x val:0x%x\n", priv->id, devad, reg, val);
		}
	}
	else if(strncmp(str[0], "help", 4) == 0) {
		printk("usage: \n"
				"echo rw $devad $reg $val > proc/xpcs\n");
	}
	else {
...
static const struct proc_ops common_ctrl = {
	.proc_write = common_proc,
};
...
static int xpcs_probe(struct platform_device *pdev)
{
	...
	sprintf(name, "xpcs%d", priv->id);
	proc_create_data(name, 0666, NULL, &common_ctrl, (void*)priv);
	printk("%s: End\n", __func__);
	return 0;
}
...
static const struct of_device_id xpcs_match[] = {
	{ .compatible = "siflower,sf21h8898-xpcs" },
	{},
};
...
MODULE_DESCRIPTION("XPCS driver for SF21H8898 SoC");
