...
struct xgmac_dma_priv *g_priv = NULL;
				...
				struct xgmac_rxq *rxq)
{
	...
	dma_wmb();
	xgmac_dma_set_rx_tail_ptr(priv, rxq->rx_tail_addr, rxq->idx);
}
...
static int xgmac_dma_poll_rx(struct xgmac_rxq *rxq, int budget)
{
	...
	for (; count < budget; count++) {
		u32 len, rdes0, rdes2, rdes3, rdes_ctx0, rdes_ctx1, rdes_ctx2, rdes_ctx3;
		struct xgmac_dma_rx_buffer *buf;
		register struct xgmac_dma_desc_rx rx;
		struct net_device *netdev;
		struct sk_buff *skb;
		unsigned int entry;
		u8 id, oid, up_reason, vlan_pri, no_frag;
		...
		rx.ctxt.des3 = __READ_ONCE(*(__le32 *)(&rxq->dma_rx[next_entry + 1].des3));
		...
		if (unlikely(rdes3 & XGMAC_RDES3_ES)) {
			sf_dbg_ratelimited("error type: 0x%lx\n",
		...
		// skb->sta_index = FIELD_GET(XGMAC_RDES0_STA_INDEX, rdes0);
		// skb->rpt_index = FIELD_GET(XGMAC_RDES0_RPT_INDEX, rdes0);
		...
		sf_dbg_ratelimited("%s: up_reason:%02x "
				"ovid:%u vlan_pri:%u no_frag:%u sport:%u eth_type:0x%x "
				"dscp:%u tnp:%u pkt_type:%u smac:%llx\n",
				netdev->name, up_reason, ovid, vlan_pri, no_frag, sport, eth_type,
				dscp, tnp, pkt_type, smac);

		xgmac_dma_rx_coe_hash(netdev, skb, rdes0, rdes_ctx1);
#ifdef CONFIG_NET_SWITCHDEV
		//skb->offload_fwd_mark = !up_reason;
#endif

		skb_record_rx_queue(skb, rxq->idx);
		skb->protocol = eth_type_trans(skb, netdev);
		oid = FIELD_GET(XGMAC_RDES0_OVPORT, rdes0);
		if (oid >= EXTDEV_OFFSET && oid <= DPNS_MAX_PORT) {
			struct net_device *dev = priv->ndevs[oid];
			if (likely(dev)) {
				skb->cb[SF_DPNS_FLAG] = SF_CB_DPNS_FORWARD;
				*((unsigned long*)(skb->cb)) = (unsigned long)dev;
			}
		}

		// skb->ipvs_property = 1;
		/* TODO: Consider using netif_receive_skb_list() instead.
		 */
	...
	for (i = 1; i < DMA_CH_MAX; i++) {
		if (BIT(i) & mtl_status) {
			...
			if (status & XGMAC_RXOVFIS)
				sf_dbg_ratelimited("RX queue %u overflow\n", i);
		...
		if (BIT(i) & dma_status) {
			...
			if (unlikely(status & XGMAC_AIS)) {
				...
				if (status & XGMAC_RBU) {
					sf_dbg_ratelimited("Trigger queue %u RBU\n", i);
				}
			}
...
static irqreturn_t xgmac_dma_irq_tx(int irq, void *dev_id)
{
	...
	struct xgmac_dma_priv *priv = container_of(txq, struct xgmac_dma_priv,
						   txq[txq->idx]);
	u32 status = reg_read(priv, XGMAC_DMA_CH_STATUS(txq->idx)) &
		     (XGMAC_TI | XGMAC_TBU);
	...
	/* Clear interrupts */
	reg_write(priv, XGMAC_DMA_CH_STATUS(txq->idx), status);
...
static irqreturn_t xgmac_dma_irq_rx(int irq, void *dev_id)
{
	...
	struct xgmac_dma_priv *priv = container_of(rxq, struct xgmac_dma_priv,
						   rxq[rxq->idx]);
	u32 status = reg_read(priv, XGMAC_DMA_CH_STATUS(rxq->idx)) &
		(XGMAC_RI);
	...
	/* Clear interrupts */
	reg_write(priv, XGMAC_DMA_CH_STATUS(rxq->idx), status);
...
static int xgmac_dma_init(struct xgmac_dma_priv *priv)
{
	...
	/* enable rx_queue 1 2 3 4 */
	regmap_write(priv->ethsys, ETHSYS_RX_QUEUE_ENABLE, 0x2A8);
...
static int xgmac_dma_init_rx_rings(struct xgmac_dma_priv *priv)
{
	...
	for (queue = 1; queue < DMA_CH_MAX; queue++) {
...
#ifdef CONFIG_NET_SIFLOWER_ETH_USE_INTERNAL_SRAM
		txq->dma_tx = gen_pool_dma_alloc(priv->genpool, DMA_TX_SIZE *
						 sizeof(*txq->dma_tx),
						 &txq->dma_tx_phy);
#else
...
{
	u16 queue = skb_get_queue_mapping(skb) + 1;
	struct xgmac_txq *txq = &priv->txq[queue];
	...
	if (unlikely(xgmac_dma_tx_avail(txq) <
		...
		sf_dbg_ratelimited("%s: Tx %d Ring full when queue awake\n", __func__, txq->idx);
		goto drop;
	}

	mss = skb_shinfo(skb)->gso_size;
	/* The header length + MSS + TxPBL must be less than Tx Queue size */
	mss = min_t(u16, mss, SZ_2K - 16 - proto_hdr_len - 1);
	...
	xgmac_dma_set_tx_tail_ptr(priv, txq->tx_tail_addr, txq->idx);
...
netdev_tx_t xgmac_dma_xmit(struct sk_buff *skb, struct xgmac_dma_priv *priv)
{
	...
	if (likely(skb->ip_summed == CHECKSUM_PARTIAL)) {
		...
		if (unlikely(skb->len >= SZ_2K - txcoeovh)) {
	...
	if (cb->fastmode)
		tdes0 = XGMAC_TDES0_FAST_MODE |
			FIELD_PREP(XGMAC_TDES0_OVPORT, cb->id) |
			FIELD_PREP(XGMAC_TDES0_QUEUE_ID, 7) |
			FIELD_PREP(XGMAC_TDES0_IVPORT, DPNS_HOST_PORT);
	else
...
netdev_tx_t xgmac_dma_xmit_fast(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (unlikely(ret != NETDEV_TX_OK)) {
		sf_dbg_ratelimited("%s: Tx Ring full when queue awake\n",
			__func__);
	}
	return ret;
}
...
// {
// 	struct wifi_to_dma *wtd;
// 	u32 cpu;

// 	/* reserve CPU3 for wifi rx  */
// 	for_each_possible_cpu(cpu) {
// 		if (cpu == 3)
// 			continue;
// 		wtd = per_cpu_ptr(&wifi_to_dma, cpu);
// 		tasklet_init(&wtd->dma_tasklet, dma_tasklet_function, (unsigned long)wtd);
// 		skb_queue_head_init(&wtd->input_pkt_queue);
// 		skb_queue_head_init(&wtd->process_queue);
// 		wtd->csd.info = wtd;
// 		wtd->csd.func = csd_func;
// 		wtd->cpu = cpu;
// 		wtd->status = STATE_IDLE;
// 	}
// }
...
static int xgmac_dma_debug_show(struct seq_file *m, void *v)
{
	...
	for (i = 1; i < DMA_CH_MAX; i++) {
		...
		seq_printf(m, "      MTL Opmode:0x%x debug:0x%x\n",
				reg_read(priv, XGMAC_MTL_TXQ_OPMODE(i)),
				reg_read(priv, XGMAC_MTL_TXQ_DEBUG(i)));
		seq_printf(m, "rxq %d curr:%d dirty:%d\n",
				i, priv->rxq[i].cur_rx, priv->rxq[i].dirty_rx);
		seq_printf(m, "      MTL Opmode:0x%x debug:0x%x\n",
				reg_read(priv, XGMAC_MTL_RXQ_OPMODE(i)),
				reg_read(priv, XGMAC_MTL_RXQ_DEBUG(i)));
		spin_unlock_bh(&(priv->txq[i].lock));

		seq_printf(m, "DMA channel %d status:0x%x debug sts:0x%x\n", i,
				reg_read(priv, XGMAC_DMA_CH_STATUS(i)),
				reg_read(priv, XGMAC_DMA_CH_DEBUG_STATUS(i)));
		seq_printf(m, "              TxDesc HAddr:0x%x TAddr:0x%x\n",
				reg_read(priv, XGMAC_DMA_CH_TxDESC_LADDR(i)),
				reg_read(priv, XGMAC_DMA_CH_TxDESC_TAIL_LPTR(i)));
		seq_printf(m, "                     Cur desAddr:0x%x bufAddr:0x%x\n",
				reg_read(priv, XGMAC_DMA_CH_CUR_TxDESC_LADDR(i)),
				reg_read(priv, XGMAC_DMA_CH_CUR_TxBUFF_LADDR(i)));
		seq_printf(m, "              RxDesc HAddr:0x%x TAddr:0x%x\n",
				reg_read(priv, XGMAC_DMA_CH_RxDESC_LADDR(i)),
				reg_read(priv, XGMAC_DMA_CH_RxDESC_TAIL_LPTR(i)));
		seq_printf(m, "                     Cur desAddr:0x%x bufAddr:0x%x\n",
				reg_read(priv, XGMAC_DMA_CH_CUR_RxDESC_LADDR(i)),
				reg_read(priv, XGMAC_DMA_CH_CUR_RxBUFF_LADDR(i)));
	}
...
static int xgmac_dma_probe(struct platform_device *pdev)
{
	...
	/* default use mapping mode 4 */
	ret = regmap_write(priv->ethsys, ETHSYS_MRI_Q_EN, 0xC000001F);
	...
	for (i = 1; i < DMA_CH_MAX; i++) {
		...
		netif_napi_add(&priv->napi_dev, &priv->txq[i].napi,
			xgmac_dma_napi_tx);
		/* txq1->cpu0
		 * txq2->cpu1
		 * txq3->cpu2
		 * txq4->cpu1 for game port
		 * cpu3 reserved for wifi
		 */
		cpu = (i == 4) ? 1: (i - 1);
		irq_set_affinity_hint(priv->txq[i].irq, cpumask_of(cpu));
	}
	...
	for (i = 1; i < DMA_CH_MAX; i++) {
		...
		priv->rxq[i].idx = i;
		netif_napi_add(&priv->napi_dev, &priv->rxq[i].napi,
			xgmac_dma_napi_rx);
		/* rxq1->cpu0
		 * rxq2->cpu1
		 * rxq3->cpu2
		 * rxq4->cpu1 for game port
		 * cpu3 reserved for wifi
		 */
		cpu = (i == 4) ? 1: (i - 1);
		irq_set_affinity_hint(priv->rxq[i].irq, cpumask_of(cpu));
	}
	...
	// debugfs_create_file("tx_stats", 0444, priv->dbgdir, priv, &xgmac_tx_stats_fops);
#endif
...
static int xgmac_dma_remove(struct platform_device *pdev)
{
...
static const struct of_device_id xgmac_dma_match[] = {
	{ .compatible = "siflower,sf21h8898-xgmac-dma" },
	{},
};
...
MODULE_DESCRIPTION("Ethernet DMA driver for SF21H8898 SoC");
