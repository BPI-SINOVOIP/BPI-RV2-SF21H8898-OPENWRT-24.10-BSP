...
static int xgmac_mdio_read(struct mii_bus *bus, int addr, int regnum)
{
	...
	reg = FIELD_PREP(MII_XGMAC_PA, addr);

	reg_set(priv, XGMAC_MDIO_C22P, BIT(addr));
	reg |= FIELD_PREP(MII_XGMAC_RA, regnum);
...
static int xgmac_mdio_c45_read(struct mii_bus *bus, int addr, int devnum, int regnum)
{
	struct xgmac_priv *priv = bus->priv;
	u32 reg;
	int ret;
	...
	reg |= FIELD_PREP(MII_XGMAC_DA, devnum) | FIELD_PREP(MII_XGMAC_RA, regnum);
...
static int xgmac_mdio_c45_write(struct mii_bus *bus, int addr, int devnum, int regnum, u16 val)
{
	struct xgmac_priv *priv = bus->priv;
	u32 reg;
	int ret;
...
static int xgmac_mdio_init(struct xgmac_priv *priv)
{
	...
	priv->mii->read_c45 = xgmac_mdio_c45_read;
	priv->mii->write_c45 = xgmac_mdio_c45_write;
			      ...
			      struct rtnl_link_stats64 *stats)
{
	...
	BUILD_BUG_ON(xgmac_mib[STATS64_TX_EXCESSIVE_DEFER].offset != MMC_XGMAC_TX_EXCESSIVE_DEFER);
	...
	stats->tx_errors = mib[STATS64_TX_EXCESSIVE_COL] +
			   mib[STATS64_TX_CARRIER] + mib[STATS64_TX_UNDER] +
			   mib[STATS64_TX_LATE_COL] + mib[STATS64_TX_EXCESSIVE_DEFER];
...
static struct phylink_pcs *sfxgmac_select_pcs(struct phylink_config *config,
...
static void xgmac_mac_link_up(struct phylink_config *config,
	...
	/* Workaround for 2500basex rate adaption.
	 * MAC will always work at 2.5G while PHY can be at a lower speed.
	 *
	 * PHY will send pause frames to MAC to adjust TX speed.
	 */
	if (interface == PHY_INTERFACE_MODE_2500BASEX && speed < SPEED_2500 &&
	    duplex == DUPLEX_FULL) {
		netdev_info(netdev, "%dBaseX rate match enabled, target speed %s\n",
			    SPEED_2500, phy_speed_to_str(speed));
		speed = SPEED_2500;
		rx_pause = true;
	} else if (interface == PHY_INTERFACE_MODE_1000BASEX && speed < SPEED_1000 &&
	    duplex == DUPLEX_FULL) {
		netdev_info(netdev, "%dBaseX rate match enabled, target speed %s\n",
			    SPEED_1000, phy_speed_to_str(speed));
		speed = SPEED_1000;
		rx_pause = true;
	}
...
static const struct phylink_mac_ops xgmac_phylink_mac_ops = {
	...
	.mac_select_pcs = sfxgmac_select_pcs,
...
static int xgmac_phy_setup(struct xgmac_priv *priv)
{
	...
	if (pcs) {
		...
		if (!phylink->phylink_disable_state &&
		    phylink->cfg_link_an_mode == MLO_AN_INBAND) {
			if (pcs->poll)
				mod_timer(&phylink->link_poll, jiffies + HZ);
			else
				del_timer(&phylink->link_poll);
		}
	}
...
static int xgmac_probe(struct platform_device *pdev)
{
	...
	ndev = devm_alloc_etherdev_mqs(&pdev->dev, sizeof(*priv),
					(DMA_CH_MAX - 1), (DMA_CH_MAX - 1));
	if (!ndev)
		return -ENOMEM;
	...
	ndev->features = NETIF_F_RXHASH | NETIF_F_RXCSUM | NETIF_F_GRO | NETIF_F_TSO |
			 NETIF_F_TSO6 | NETIF_F_SG | NETIF_F_LLTX | NETIF_F_HW_TC |
...
static const struct of_device_id xgmac_match[] = {
	{ .compatible = "siflower,sf21h8898-xgmac" },
	{},
};
...
MODULE_DESCRIPTION("Ethernet XGMAC driver for SF21H8898 SoC");
