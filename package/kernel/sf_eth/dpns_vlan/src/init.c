...
void dynamic_vlan_update(VLAN_t *priv)
{
	...
	for (port = 0; port < DPNS_MAX_PORT; port++){
		...
		if (count == PORT_WITH_NO_VLAN) {
			evlan_xlt_table_update(priv, port, port, 1, 0, 0, 0, 0, 1, 1, 1);
		} else if (count == PORT_WITH_ONE_VLAN) {
			evlan_xlt_table_update(priv, port, port, 1, 0, 0, 0, vlan_id, 1, 1, 0);
		} else {
			evlan_xlt_table_update(priv, port, 0, 0, 0, 0, 0, 0, 0, 0, 0);
			untagged_ports |= BIT(port);
		}
	...
	evlan_lkp_table_update(priv, 0, 0, 1, untagged_ports, 0x7ffffff);

	spin_lock_bh(&priv->vlan_lock);
	list_for_each_entry(pos, &priv->vlan_list, node) {
		VLAN_DBG(DBG_LV, "%s ilkp_idx:%u elkp_idx:%u vid:%u ports:0x%x un_ports:0x%x\n",
			 __func__,pos->ivlan_lkp_index, pos->evlan_lkp_index,
			 pos->vlan_id, priv->member_ports, untagged_ports);

		pos->untagged_ports = untagged_ports;
