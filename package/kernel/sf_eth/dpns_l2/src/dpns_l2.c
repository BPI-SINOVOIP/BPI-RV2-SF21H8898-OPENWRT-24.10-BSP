...
int sf_mac_del_entry(MAC_t* priv, const u8 *dmac, u16 vlan_id, bool is_deled_by_event, bool is_netdev_event)
{
	...
	if (!dev && !is_netdev_event) {
		L2_DBG(DBG_LV, "this is the dev's mac\n");
		return 0;
	}

	// the gateway's mac can't del by user.
				  ...
				  unsigned long event, void *ptr)
{
	...
	COMMON_t *cpriv = priv->cpriv;
	bool learning = !fdb_info->added_by_user && !fdb_info->is_local;
	u8 port_id;
	u8 l3_en = 0;
...
int se_l2_set_term_mac(MAC_t *priv, const u8 *mac, u16 vlan_id, bool is_add)
{
	int err = 0;
	...
	if (!is_add) {
		...
		err = sf_mac_del_entry(priv, mac, vlan_id ?: DPA_UNTAGGED_VID, true, true);
		return err;
	}
				  ...
				  unsigned long event, void *ptr)
{
	...
	case NETDEV_CHANGEADDR:
		...
		err = se_l2_set_term_mac(priv, dev->dev_addr, vlan_id, true);
